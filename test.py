...
with open("userid.csv", encoding="utf-8") as f:
    ...
    for id in idlist:
        ...
        for game in usergamelist:
            ...
            if medplaytime4ever != 0:
                score = (
                    np.log(1 + gameplaytime4ever) / np.log(1 + (medplaytime4ever + k))
                    + 0.3 * np.log(1 + gameplaytime2weeks) / np.log(1 + medplaytime2weeks)
                    if medplaytime2weeks > 0
                    else np.log(1 + gameplaytime4ever) / np.log(1 + (medplaytime4ever + k))
                )
                gamedata.append(
                    [
                        gameid,
        ...
        for game in userprofil[1]:
            game_profil = {"Utilisateur": id, "Nom": game[0][1], "Score": float(game[0][6])}
            for label in VALIDTAG:
                if label in game[1].keys():
                    game_profil.update(
                        {label: round(game[1].get(label) / sum(list(game[1].values())), 2)}
                    )
                else:
